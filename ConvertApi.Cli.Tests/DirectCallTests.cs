...
public class DirectCallTests
{
    ...
    [Test]
    public async Task TestSetOutputFileName()
    {
        var inputFile = Path.Combine(Directory.GetCurrentDirectory(), "../../../../", "test_files", "simple.pdf");
        var copyOfInputFile = Path.Combine(TestOutputDir, "simple.pdf");
        File.Copy(inputFile, copyOfInputFile);
        var outputFile = Path.Combine(TestOutputDir, "simple2.pdf");

        await Program.Main([ApiToken, outputFile, copyOfInputFile, "pdf", "repair"]);

        Assert.IsTrue(File.Exists(outputFile), "Output file was not created.");
    }

    [Test]
    public async Task TestGenerateSafeFiles()
    {
        var outputFile = Path.Combine(TestOutputDir, "output.png");
